...
- 前台：首页、文章详情、归档、分类、标签、分页搜索、相关文章、RSS 与关于页
- 后台：登录、文章管理、版本恢复、限时预览链接、封面管理、分类标签、评论审核、分析看板与站点设置
- Markdown 写作：实时预览、编辑建议、本地草稿恢复
- Glyph 封面：标题首字母与文章章节、代码、引用、列表、图片共同生成确定性封面，首次发布后锁定
- 中文标题拼音 slug 生成
- 评论审核：回复、蜜罐字段、垃圾评论评分与数据库原子限流
- 搜索：标题、slug、摘要、正文、分类、标签综合匹配并按相关度排序，记录零结果选题线索
- 隐私统计：按 UTC 日聚合页面访问、阅读深度、搜索和来源域名，不保留请求级 PageView、IP 或 User-Agent
- 主题切换：亮色/暗色
- 公开内容缓存：请求内去重与跨请求标签缓存，后台写入后立即失效；生产构建不依赖数据库在线
...
- `TRUST_PROXY_HEADERS`：只有受信任反向代理会覆盖转发头时才设为 `true`。仓库内的 Nginx 自托管流程要求生产值为 `true`；本地开发保持 `false`。
- `MANUAL_COVER_HOSTS`：生产环境允许使用人工封面的精确主机名，逗号分隔且不支持通配符，例如 `cdn.example.com,images.example.com`。未配置时生产环境会明确拒绝 MANUAL 封面。
...
根布局保留运行时动态边界，因此 `npm run build` 不要求生产数据库在线。公开文章、归档、分类、标签、RSS、sitemap 与站点设置使用带标签的一小时跨请求缓存，后台写入会立即失效相关标签；搜索只做请求内去重，避免不可信查询制造持久缓存键。生产服务启动后必须能访问 `DATABASE_URL`。
...
5. 在任何迁移前审计旧封面和 Glyph 候选：

```sh
NODE_ENV=production node .next/deploy-tools/backfill-glyph-recipes.mjs --preflight
```

6. 停止旧应用或启用会阻断写入的维护模式，然后对生产数据库执行迁移：

必须在迁移前阻断旧版本写入。否则旧代码可能在新增封面字段后继续保存文章，产生错误的默认封面模式。迁移、回填或切换失败时，应恢复旧 `current` 并重启旧应用。

```sh
npx prisma migrate deploy
```

7. 执行幂等的 Glyph 历史封面回填：

```sh
NODE_ENV=production node .next/deploy-tools/backfill-glyph-recipes.mjs
```
...
```sh
NODE_ENV=production node .next/deploy-tools/backfill-glyph-recipes.mjs --maintenance
```

`--maintenance` 会清空回滚兼容表中的请求级数据，并删除超过 400 天的分析聚合、过期限流 bucket 和预览令牌。自托管服务器还应通过系统 cron 每天执行一次；仅依赖有访问时的有界清理不能提供严格的保留期。
...
5. 检查通过后，GitHub Actions 执行 `npm run build:release`，生成 `.next/standalone` 和自包含的 Glyph 回填工具。
6. GitHub Actions 通过 SSH 上传产物压缩包到服务器。
7. 服务器解压到 `DEPLOY_PATH/releases/<commit-sha>`，复制服务器本地维护的 `.env` / `.env.production`。
8. 服务器先执行只读 preflight，再暂停旧 PM2 应用以阻断写入，然后执行生产数据库迁移；幂等 Glyph 回填成功后才切换 `DEPLOY_PATH/current`。维护窗口内失败会恢复旧链接并重启旧应用，新版本启动后再运行维护清理。
...
- 自托管 `.env` 已设置 `TRUST_PROXY_HEADERS=true`；使用人工封面时，`MANUAL_COVER_HOSTS` 只填写逗号分隔的 hostname，不含 scheme、端口或通配符，修改后需要重启应用。
- Nginx 反向代理到 PM2 使用的 `127.0.0.1:3000`，并覆盖 `X-Forwarded-For` / `X-Real-IP` 为连接方地址；或按需调整 workflow 中的 `PORT`。
...
不要使用 `$proxy_add_x_forwarded_for` 配合 `TRUST_PROXY_HEADERS=true`；应用读取第一个地址，追加模式会让客户端伪造限流身份。
...
NODE_ENV=production node deploy-tools/backfill-glyph-recipes.mjs --preflight
pm2 stop "$PM2_APP_NAME"
node node_modules/prisma/build/index.js migrate deploy --config ./prisma.config.ts
NODE_ENV=production node deploy-tools/backfill-glyph-recipes.mjs
ln -sfn "$RELEASE_DIR" "$DEPLOY_PATH/current"
pm2 startOrReload ecosystem.config.cjs --update-env
NODE_ENV=production node deploy-tools/backfill-glyph-recipes.mjs --maintenance
```

`deploy-tools/backfill-glyph-recipes.mjs` 在 GitHub Actions 构建阶段打成单文件，并在一个不含 `node_modules` 的临时目录执行 `--help` 自检。release 只携带 Next.js standalone 追踪依赖、Prisma CLI 依赖闭包和该工具，不会打包完整的开发期 `node_modules`。生产服务器不需要执行 `npm install`，也不需要访问 npm registry。preflight 会在迁移前审计旧人工封面并验证 Glyph 候选；回填只处理缺少有效配方或 hash 不一致的已发布 GLYPH 文章。迁移只清空旧请求级数据但暂时保留回滚兼容列，物理删列留给后续稳定版本。自动部署会在迁移前暂停旧应用，避免旧代码在新增字段或回填窗口继续写入；若迁移、回填或切换失败，退出 trap 会恢复旧 `current` 并重启旧 PM2 应用。
...
npm run platform:maintenance
npm run prisma:generate
npm run prisma:backfill-glyphs
npm run prisma:migrate
npm run prisma:seed
...
- 生产人工封面必须命中 `MANUAL_COVER_HOSTS`；Markdown 图片由可信作者控制，并使用 `no-referrer` 浏览器直连。
- 分析数据只保存有界的每日聚合；严格 400 天保留依赖每日执行 `platform:maintenance`。
- 测试数据库必须和开发/生产数据库隔离。
