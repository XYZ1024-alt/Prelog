...
- 后台：登录、文章管理、版本恢复、限时预览链接、分类标签、评论审核、分析看板与站点设置
...
5. 停止旧应用或启用会阻断写入的维护模式，然后对生产数据库执行迁移：

必须在迁移前阻断旧版本写入，避免旧代码在 schema 变更后继续写入不兼容的数据。迁移或切换失败时，应恢复旧 `current` 并重启旧应用。
...
```sh
NODE_ENV=production node .next/deploy-tools/maintenance.mjs
```

维护工具会清空回滚兼容表中的请求级数据，并删除超过 400 天的分析聚合、过期限流 bucket 和预览令牌。自托管服务器还应通过系统 cron 每天执行一次；仅依赖有访问时的有界清理不能提供严格的保留期。
...
5. 检查通过后，GitHub Actions 执行 `npm run build:release`，生成 `.next/standalone` 和自包含的维护工具。
6. GitHub Actions 通过 SSH 上传产物压缩包到服务器。
7. 服务器解压到 `DEPLOY_PATH/releases/<commit-sha>`，复制服务器本地维护的 `.env` / `.env.production`。
8. 服务器暂停旧 PM2 应用以阻断写入，然后执行生产数据库迁移，成功后切换 `DEPLOY_PATH/current`。维护窗口内失败会恢复旧链接并重启旧应用，新版本启动后再运行维护清理。
...
- 自托管 `.env` 已设置 `TRUST_PROXY_HEADERS=true`。
...
NODE_ENV=production node deploy-tools/maintenance.mjs
```

`deploy-tools/maintenance.mjs` 在 GitHub Actions 构建阶段打成单文件，并在一个不含 `node_modules` 的临时目录执行 `--help` 自检。release 只携带 Next.js standalone 追踪依赖、Prisma CLI 依赖闭包和该工具，不会打包完整的开发期 `node_modules`。生产服务器不需要执行 `npm install`，也不需要访问 npm registry。维护工具会清空回滚兼容的请求级数据，并删除超过 400 天的分析聚合、过期限流 bucket 和预览令牌。自动部署会在迁移前暂停旧应用，避免旧代码在 schema 变更后继续写入；若迁移或切换失败，退出 trap 会恢复旧 `current` 并重启旧 PM2 应用。
...
- Markdown 图片由可信作者控制，并使用 `no-referrer` 浏览器直连。
